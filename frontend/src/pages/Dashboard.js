...
export default function Dashboard() {
  ...
  const DIFFICULTIES = [t.all, t.beginner, t.intermediate, t.expert];
  const TYPES = [t.all, t.indoor, t.outdoor];
  ...
  const filtered = plants.filter(p => {
    ...
    const diffMap = { [t.beginner]:'Beginner', [t.intermediate]:'Intermediate', [t.expert]:'Expert' };
    const typeMap = { [t.indoor]:'Indoor', [t.outdoor]:'Outdoor' };
    const matchDiff = filterDiff===t.all || p.difficulty===diffMap[filterDiff];
    const matchType = filterType===t.all || (p.care_guide?.type||'').toLowerCase().includes(typeMap[filterType]?.toLowerCase()||'');
    return matchSearch && matchDiff && matchType;
  });

  const dc = {Beginner:'badge-green',Intermediate:'badge-amber',Expert:'badge-red'};
  const hasFilters = filterDiff!==t.all || filterType!==t.all;

  return (
    <div style={s.page}>
      {/* Navbar */}
      <nav style={s.nav}>
        <div style={s.nl}><span style={{fontSize:24}}>🌿</span><span style={s.nb}>PlantWise</span></div>
        <div style={s.nr}>
          {/* Language Switcher */}
            ...
            {showLangMenu && (
              <>
                <div style={s.langDrop}>
                  {LANGS.map(l => (
                    <button key={l.code} style={{...s.langOpt,...(lang===l.code?s.langOptOn:{})}}
                      onClick={()=>{ switchLang(l.code); setShowLangMenu(false); }}>
                      <span style={{fontSize:16}}>{l.label}</span>
                      <span style={{fontSize:13}}>{l.full}</span>
                      {lang===l.code && <span style={{marginLeft:'auto',color:'var(--mint)'}}>✓</span>}
                    </button>
                  ))}
                </div>
                <div style={s.langBackdrop} onClick={()=>setShowLangMenu(false)}/>
              </>
            )}
          </div>
      ...
      <div style={s.content}>
        ...
        <div style={s.hdr} className="animate-fadeUp">
          <div>
            ...
            <p style={s.sub}>
              {plants.length===0 ? t.startByAdding
          ...
          <div style={s.filterBar} className="animate-fadeUp">
            ...
            <div style={s.filterChips}>
              <div style={s.filterGroup}>
                {DIFFICULTIES.map(d=>(
                  <button key={d} style={{...s.chip,...(filterDiff===d?s.chipOn:{})}} onClick={()=>setFilterDiff(d)}>{d}</button>
                ))}
              </div>
              <div style={s.filterGroup}>
                {TYPES.map(tp=>(
                  <button key={tp} style={{...s.chip,...(filterType===tp?s.chipOn:{})}} onClick={()=>setFilterType(tp)}>{tp}</button>
                ))}
              </div>
            </div>
            {hasFilters && <button style={s.clearBtn} onClick={()=>{setFilterDiff(t.all);setFilterType(t.all);}}>{t.clearFilters}</button>}
          </div>
        )}

        {/* Grid */}
        {loading ? (
          <div style={s.ls}><div className="spinner" style={{width:32,height:32}}/><p style={{color:'var(--text-light)',marginTop:16}}>{t.loadingGarden}</p></div>
          ...
          <div style={s.es} className="animate-fadeUp">
            ...
            <button style={{...s.clearBtn,marginTop:16}} onClick={()=>{setSearch('');setFilterDiff(t.all);setFilterType(t.all);}}>{t.clearAll}</button>
          </div>
        ) : (
          <div style={s.grid}>
            {filtered.map((p,i)=>(
              <div key={p.id} className="card animate-fadeUp"
                ...
                <div style={s.ci}>
                  ...
                  <div style={s.actions} className="card-actions">
                    <button style={{...s.actionBtn,...(wateringId===p.id?s.actionBtnSuccess:{})}} onClick={e=>handleWaterLog(e,p)} title={t.waterLog}>
                      {wateringId===p.id?'✓':'💧'}
                    </button>
                    <button style={{...s.actionBtn,...s.actionBtnDanger}} onClick={e=>{e.stopPropagation();setDeleteConfirm(p.id);}} title={t.delete}>🗑️</button>
                  </div>
                </div>
                <div style={s.ci2}>
...
const s = {
  ...
  nr:          {display:'flex',alignItems:'center',gap:10},
  ...
  langBtn:     {background:'var(--card-bg)',border:'1px solid var(--border)',borderRadius:100,padding:'6px 12px',color:'var(--text-dark)',fontSize:13,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',gap:5},
  langDrop:    {position:'absolute',top:44,right:0,background:'var(--card-bg)',border:'1px solid var(--border-strong)',borderRadius:14,padding:6,minWidth:160,zIndex:300,boxShadow:'0 8px 32px rgba(0,0,0,0.4)'},
  langOpt:     {display:'flex',alignItems:'center',gap:10,width:'100%',padding:'10px 12px',borderRadius:10,border:'none',background:'none',cursor:'pointer',color:'var(--text-dark)',fontSize:14,transition:'background 0.15s'},
  langOptOn:   {background:'rgba(82,183,136,0.12)'},
  langBackdrop:{position:'fixed',inset:0,zIndex:299},
