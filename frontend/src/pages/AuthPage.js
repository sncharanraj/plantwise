  ...
  return (
    <div style={s.container}>
      <div style={s.left}>
        ...
        <div style={s.credit}>Made with <span style={s.heart}>♥</span> by S N Charanraj</div>
      </div>
      ...
      <div style={s.right}>
        ...
        <div style={s.langRow}>
          {LANGS.map(l => (
            <button key={l.code} style={{ ...s.langBtn, ...(lang===l.code?s.langBtnActive:{}) }} onClick={() => switchLang(l.code)}>
              {l.label}
            </button>
          ))}
        </div>
        ...
        <div style={s.card} className="animate-growIn">
          <h2 style={s.formTitle}>{mode==='login' ? t.welcomeBack : t.startGrowing}</h2>
          <p style={s.sub}>{mode==='login' ? t.signInToGarden : t.createFreeAccount}</p>

          {error && <div style={s.err}>{error}</div>}
          {success && <div style={s.suc}>{success}</div>}

          <form onSubmit={handleSubmit} style={s.form}>
            <input className="input" type="email" placeholder={t.emailPlaceholder} value={email} onChange={e=>setEmail(e.target.value)} required style={{marginBottom:12}}/>
            <input className="input" type="password" placeholder={t.passwordPlaceholder} value={password} onChange={e=>setPassword(e.target.value)} required minLength={6} style={{marginBottom:20}}/>
          ...
          <p style={s.sw}>
            ...
            <button style={s.swb} onClick={()=>{setMode(mode==='login'?'signup':'login');setError('');}}>
              {mode==='login' ? t.signUpFree : t.signIn}
            </button>
          </p>
        </div>
        <div style={s.creditMobile}>Made with <span style={s.heart}>♥</span> by S N Charanraj</div>
      </div>
    </div>
  );
}
...
const s = {
  ...
  langBtn:{padding:'6px 14px',borderRadius:100,border:'1px solid var(--border)',background:'transparent',cursor:'pointer',fontSize:14,color:'var(--text-mid)',transition:'all 0.2s'},
